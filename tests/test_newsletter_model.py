...
def test_arc_fields_present_and_leftovers_removed():
    ...
    # new arc blocks
    for f in ("selection_headline", "architecture", "a_emporter", "verdict", "framing"):
        assert f in fields
    # leftover fields that no longer map to a section are gone
    for f in ("wrap_up", "prolongements", "open_question", "angles_morts", "cui_bono"):
...
def test_verdict_uses_lists_and_drops_tient_fragile():
    ...
    assert set(("enjeux", "objections", "angles_morts", "nuances")) <= set(fields)
    for gone in ("enjeu", "objection", "tient_fragile", "la_question", "questions"):
