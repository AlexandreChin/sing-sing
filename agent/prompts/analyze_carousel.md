...
Produis cinq listes, 1 à 3 items chacune, 1 phrase par item :
...
Produis 3 à 5 items. Chaque item :
- `aspect` (1 mot)
- `summary` (1 phrase, 2 maximum, sans citation)
- `seeds` : quel élément de `before_you_read` a planté cette observation. Format : `"<catégorie>: <début de l'item concerné>"`. Catégories valides : `context`, `important_fact`, `watch_out`. Exemple : `"watch_out: 'Observez combien de fois B'Tselem…'"`.
...
- `seeds` : quel élément de `before_you_read` a planté cet item. Même format que pour les observations.
