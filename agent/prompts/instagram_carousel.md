...
- `global_analysis` : l'**architecture de l'argument** — le regard GLOBAL sur le raisonnement de l'article (vs les slides 4–6, qui sont des annotations LOCALES). PAS un récapitulatif des faits (ça, c'est `key_takeaways`, slide 9 — ne le répète pas). NE CONCLUS PAS à la place du lecteur : décris comment l'argument est bâti, puis désigne le maillon décisif à tester, formulé en QUESTION ouverte (jamais « non démontré / glissement / saut » ni un verdict). Appuie-toi sur `fond.logical_reasoning` et `fond.implicit_assumptions`. `headline` (≤10 mots — une question qui invite à examiner la thèse). `core_recap` (2 points, ≤16 mots : §1 = le fil du raisonnement décrit NEUTREMENT, souvent en chaîne `A → B → C` ; §2 = le maillon/l'hypothèse qui porte tout, posé comme une question que le lecteur tranche lui-même).
