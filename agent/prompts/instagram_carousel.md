...
- `global_analysis` : la **mise en perspective** — remettre les chiffres/claims de l'article à leur ÉCHELLE, PAS un récapitulatif des faits (ça, c'est `key_takeaways`, slide 9 — ne le répète pas). Donne le dénominateur ou la proportion qui manque, distingue les échelles (local vs global, relatif vs absolu, individuel vs systémique), situe l'ampleur réelle. Appuie-toi sur `annotations`/`fond` (omissions, angles morts, effets de base, part jamais chiffrée). `headline` (≤10 mots — l'invitation à prendre de la hauteur, neutre). `core_recap` (2–3 points, ≤16 mots — des points de proportion/d'échelle GROUNDÉS dans l'analyse, pas les mêmes faits que `key_takeaways`). `note` (≤18 mots — le réflexe de proportion, neutre, sans verdict).
