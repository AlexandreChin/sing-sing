...
- `go_further` : 2 à 3 items `{title, source, why}` — **Pour aller plus loin** : des ressources qui complètent l'article (comblent un angle mort, éclairent un chiffre). `why` = 1–2 phrases. Inspire-toi de `review.verdict.further_resource` et des angles morts.
